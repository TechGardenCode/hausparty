...
RETURNS TABLE(artist1_id uuid, artist1_name text, artist2_id uuid, artist2_name text, sim real) LANGUAGE sql STABLE AS $$
  ...
  FROM "artists" a1 CROSS JOIN "artists" a2
  WHERE a1."id" < a2."id" AND similarity(a1."name", a2."name") > similarity_threshold
...
BEGIN
  SELECT "name", COALESCE("aliases", '{}') INTO dup_name, dup_aliases FROM "artists" WHERE "id" = duplicate_id;
  IF dup_name IS NULL THEN RAISE EXCEPTION 'Duplicate artist % not found', duplicate_id; END IF;

  SELECT COALESCE("aliases", '{}') INTO canonical_aliases FROM "artists" WHERE "id" = canonical_id;
  IF canonical_aliases IS NULL THEN RAISE EXCEPTION 'Canonical artist % not found', canonical_id; END IF;

  UPDATE "artists" SET "aliases" = (
    SELECT array_agg(DISTINCT val) FROM unnest(canonical_aliases || dup_aliases || ARRAY[dup_name]) AS val WHERE val IS NOT NULL
  ) WHERE "id" = canonical_id;

  UPDATE "set_artists" SET "artist_id" = canonical_id
  WHERE "artist_id" = duplicate_id AND "set_id" NOT IN (SELECT "set_id" FROM "set_artists" WHERE "artist_id" = canonical_id);

  DELETE FROM "set_artists" WHERE "artist_id" = duplicate_id;

  INSERT INTO "artist_genres" ("artist_id", "genre_id")
  SELECT canonical_id, "genre_id" FROM "artist_genres" WHERE "artist_id" = duplicate_id ON CONFLICT DO NOTHING;
  DELETE FROM "artist_genres" WHERE "artist_id" = duplicate_id;

  UPDATE "scraper_entity_map" SET "internal_id" = canonical_id, "updated_at" = now()
